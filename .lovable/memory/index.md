...
App name: Zippy (reliable service) - logo and banner in src/assets/
Locale: Uzbek (O'zbek) for all UI labels.
Zones: Gurlan city zones (loaded from DB)
...
Auth: Username-based login (username@gurlan.taxi internal email), driver registration requires admin approval
KM tracking: Starts counting when driver begins trip (IN_TRIP status)
Admin credentials: username: admin, password: admin123
All pages connected to real Supabase (Lovable Cloud) database
Realtime: Orders table has realtime subscriptions for operator and driver pages
